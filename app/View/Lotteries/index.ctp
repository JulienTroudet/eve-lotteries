...
<script>
    ...
    function updateLotteriesCountDown(time_now) {
        ...
        $( ".lot-timer" ).each(function( index ) {
            ...
            $(this).html('Started since '+moment.utc(start_date).countdown(time_now).toString());
        });

        time_now.add(1, 'seconds');

        setTimeout(function () {updateLotteriesCountDown(time_now)}, 1000 );
    }
</script>